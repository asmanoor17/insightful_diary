class JournalsController < ApplicationController
  ...
  def create
    # Ensure that the user specified in journal_params exists
    user_id = journal_params[:user_id]
    unless User.exists?(user_id)
      flash[:alert] = "User with ID #{user_id} does not exist."
      redirect_to new_journal_path and return
    end
  ...
  private
    ...
    def journal_params
      params.require(:journal).permit(:content, :user_id)
    end
end
